...
Write-Host "[1/7] Checking repository contracts and privacy boundaries..." -ForegroundColor Cyan
& $python (Join-Path $PSScriptRoot "check_harness.py")
Assert-ExitCode "Repository Harness"

Write-Host "[2/7] Running Harness unit tests..." -ForegroundColor Cyan
& $python -m unittest discover -s (Join-Path $root "tests") -p "test_*.py"
Assert-ExitCode "Harness unit tests"

Write-Host "[3/7] Validating the deterministic RAG dataset..." -ForegroundColor Cyan
& $python (Join-Path $PSScriptRoot "rag_eval.py") validate
Assert-ExitCode "RAG dataset validation"

Write-Host "[4/7] Scoring the RAG Harness smoke predictions..." -ForegroundColor Cyan
& $python (Join-Path $PSScriptRoot "rag_eval.py") score
Assert-ExitCode "RAG Harness smoke score"

Write-Host "[5/7] Running Spring tests when the backend exists..." -ForegroundColor Cyan
...
Write-Host "[6/7] Running Vue tests and build when the frontend exists..." -ForegroundColor Cyan
...
Write-Host "[7/7] Validating Docker Compose configuration when available..." -ForegroundColor Cyan
